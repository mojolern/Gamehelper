namespace RuneforgeHelper
{
    ...
    //
    // deviceâ†’station path (no heap scan, persists out of the network bubble â€” docs/re-findings.md Â§6.10):
    //   device â†’ StateMachine component (SM) â†’ listener vector at SM+0x20 (begin/end)
    //          â†’ for each node ptr N: station = *(N) âˆ’ 0x98, verified by *(station+0x10) == device addr
    // anchor (docs Â§6.8/6.10):
    //   station+0x28 = rune row ptr; index = (rowPtr âˆ’ runeTableBase)/0x6c (runeTableBase = *(*(station+0x30+0x28)))
    //   station+0x3c = anchor hole index (0-based)
    public sealed partial class RuneforgeHelperCore
    {
        ...
        private const uint ColorPriceBg = 0xE6000000u;

        private const int StationOwnerOffset = 0x10;      // -> device entity
        private const int StationAnchorRefOffset = 0x28;  // â†’ Expedition2Runes row ptr
        private const int StationAnchorHolderOffset = 0x30;
        private const int StationHoleCountOffset = 0x38;   // â†’ N (recipe hole count)
        private const int StationAnchorPosOffset = 0x3c;   // â†’ anchor hole index (0..5)
        ...
        private readonly List<double> monoPriceScratch = new(); // per-reward totals â†’ row-total colour median
        private Dictionary<int, string> runeNames = new();
        // (anchorRune, pos1based, size) â†’ min area level at which that partial size is offered.
        ...
        // â”€â”€ recipe data (offline) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        private bool LoadMonolithData()
        {
            ...
            {
                Console.WriteLine($"[RuneforgeHelper] recipe json load failed: {ex.Message}");
                return false;
            }
        }

        // â”€â”€ window â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // Entry point: loads the catalog, rescans nearby monoliths on a timer, then draws whichever of
        // the two windows are enabled â€” the rewards list (ShowMonolithRewards) and/or the per-monolith
        // debug dump (ShowMonolithDebugWindow). Both read the same scanned this.monolithViews.
        private void DrawMonolithRewards()
        {
            ...
            if (this.Settings.ShowMonolithDebugWindow) this.DrawMonolithDebugWindow();
        }
        ...
        // projected map position â€” the same spot Radar paints the socket count. Radar can't be modified and
        ...
        private void DrawMonolithMapLabels()
        {
            ...
            // maxBest across visible monoliths â€” needed by the ColorMode=Relative header tint so the map
            ...
            foreach (var v in this.monolithViews)
            {
                ...
                // Same tint as the rewards-window header (shared helper); untinted â†’ white on the map.
        ...
        private void DrawMonolithRewardsWindow()
        {
            ...
            if (ImGui.Begin("Monolith Rewards", ImGuiWindowFlags.AlwaysAutoResize))
            {
                ...
                foreach (var v in this.monolithViews)
                {
                    ...
                    string hdr;
                    if (v.IsUnique)
                        hdr = $"Unique Monolith  Â·  {v.HoleCount} holes  Â·  {v.Distance:F0}  Â·  best {best:F0} ex###m{v.EntityId}";
                    else if (v.AnchorIdx >= 0)
                        hdr = $"{v.AnchorName}  Â·  hole {v.AnchorPos + 1}/{v.HoleCount}  Â·  {v.Distance:F0}  Â·  best {best:F0} ex###m{v.EntityId}";
                    else
                        hdr = $"(anchor ?)  Â·  {v.HoleCount} holes  Â·  {v.Distance:F0}###m{v.EntityId}";
                            ...
                            new Vector2(430f, 0f)))
                    {
                        ...
                        foreach (var c in v.Candidates)
                        {
                            ...
                            ImGui.Text(c.Priced ? c.UnitEx.ToString("F0") : "â€”");
                            ImGui.TableSetColumnIndex(3);
                            if (c.Priced && colorize)
                            {
                                ImGui.PushStyleColor(ImGuiCol.Text, this.PickColor(total, median, true));
                                ImGui.Text(total.ToString("F0"));
                                ImGui.PopStyleColor();
                            }
                            else
                            {
                                ImGui.Text(c.Priced ? total.ToString("F0") : "â€”");
                            }
                            shown++;
                        }
        ...
        // â”€â”€ debug window (repurposes the "Show debug list window" toggle) â”€â”€â”€â”€â”€â”€
        ...
        private void DrawMonolithDebugWindow()
        {
            ...
            ImGui.Separator();
            if (v.IsUnique)
                ImGui.Text($"Unique monolith (no anchor) â€” offers all recipes with size <= N ({v.HoleCount}).");
            else if (v.AnchorIdx < 0)
            {
                ImGui.TextColored(red, "Anchor not resolved â€” no recipes.");
            ...
            // N: station +0x38 vs StateMachine "sockets" â€” flag the under-read case.
        ...
        // Mark the anchor hole in a " Â· "-joined rune list: anchor at pos 1 of "a Â· b Â· c" â†’ "a Â· [b] Â· c".
        private static string MarkAnchor(string runes, int pos)
        {
            if (string.IsNullOrEmpty(runes) || pos < 0) return runes;
            var parts = runes.Split(" Â· ");
        ...
        // â”€â”€ enumeration / resolution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        private List<MonoView> EnumerateMonoliths()
        {
            ...
            foreach (var kv in area.AwakeEntities)
            {
                ...
                // with IsValid + Life 100/100 â€” docs Â§6.11); the "collected" signal is the StateMachine
                ...
                // Observed values (live, docs Â§6.11): 0 = dormant/out of range, 1 = available; collected
                ...
                if (this.TryResolveStation(sm, e.Address, out var station, out var sdiag))
                {
                    ...
                    // Read the station fields whenever it resolves â€” independent of the anchor. N (hole
                    ...
                    // always carry one (docs Â§6.6), so a null anchor is the discriminator under which the
                    // game's offer builder runs its skip-anchor branch (offers all size<=N; docs Â§6.12).
        ...
        // anchor rune index = (station+0x28 row ptr âˆ’ table base)/0x6c; hole pos = station+0x3c.
        ...
        private bool TryReadAnchor(IntPtr station, out int idx, out int pos, out string diag)
        {
            ...
            // makes rowPtr âˆ’ base go negative on the next map (mirrors FUN_14179ed70 exactly).
        ...
        // There is NO category/theme filter â€” the "theme" is emergent from which rune sits at which hole.
        ...
        // FUN_141e33b00 (docs/re-findings.md Â§6.12): when the station carries no pre-placed anchor
        ...
        // So it offers the whole catalog that fits the holes â€” hence the very large list.
        ...
        private void AddCandidate(MonoView v, MonoRecipe rec)
        {
            var c = new MonoCand
            {
                ...
                Runes = rec.runes != null ? string.Join(" Â· ", rec.runes) : string.Empty,
                Row = rec.row,
        ...
        // `pos` (0-based) given the current area level. areaLevel<=0 means level unknown â†’ don't gate on it.
        ...
        // top tier (â‰¥0.5Ã—max), red for the long cheap tail (â‰¤0.2Ã—max), yellow in between â€” robust to
        ...
        // scheme (green â‰¥ T, yellow 0.6Ã—T..T, untinted below); otherwise the ColorMode-driven header tint.
        ...
        private uint MonolithValueColor(double best, double maxBest, out bool tinted)
        {
            ...
            if (threshold > 0f)
            {
                ...
                return ColorWhite; // below 0.6Ã—threshold â†’ untinted
            }
        ...
        private uint PickHeaderColor(double best, double maxBest)
        {
            if (this.Settings.ColorMode == RewardColorMode.Absolute)
                return this.PickColor(best, 0, true);
        ...
        private sealed class MonoView
        {
            ...
            public int HoleCount;          // N â€” authoritative, from station +0x38
            ...
            public bool IsUnique;          // anchor-less "unique" monolith: offers all size<=N (docs Â§6.12)
